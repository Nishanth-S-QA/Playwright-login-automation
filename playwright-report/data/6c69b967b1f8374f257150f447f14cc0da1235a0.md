...
- Name: login.spec.js >> Login Workflow - demoblaze.com Authentication Suite >> TC01 - Successful login with valid credentials
...
```ts
  ...
  3  |     this.page = page;
  4  | 
  5  |     // Nav elements that open the login modal
  6  |     this.loginNavLink = page.locator('#login2');
  7  | 
  8  |     // Modal elements
  9  |     this.usernameInput = page.locator('#loginusername');
  10 |     this.passwordInput = page.locator('#loginpassword');
  11 |     this.submitButton = page.locator("button[onclick='logIn()']");
  12 |   }
  13 | 
  14 |   async goto() {
  15 |     await this.page.goto('/index.html');
  16 |   }
  17 | 
  18 |   async openLoginModal() {
> 19 |     await this.loginNavLink.click();
     |                             ^ Error: locator.click: Test timeout of 30000ms exceeded.
  20 |     await this.usernameInput.waitFor({ state: 'visible' });
  21 |   }
  22 | 
  23 |   async login(username, password) {
  24 |     await this.openLoginModal();
